...
AK04 b)
Ammerer Jakob
LIBs in Use: libao 1.2.0 URL:https://xiph.org/ao/
*/
...
// this struct instance was generated by CLAUDE based on a notessheet of said melody and the midi numbers for the base tones
...
float midiToFreq(int octave, int semitone); // this function was used to get the specific frequencies for my tones for the melody
...
void playNote(ao_device *device, float freq, float durationInSecs) {
	...
	float amp = 0.75; 											// amplitude 75% to prevent clipping if sin is +- 1
 	int samplesCount = (int)(SAMPLE_RATE * durationInSecs); 	// Sample Count for each note that is passed in function e.g:
 	...
 	for(int i = 0; i < samplesCount; i++){
		if(freq > 0){
			...
			// this is used to "pause". In line 129 the buffer get filled with zeros which means silence
			sample = (int)(amp * bit16audio * sin(2 * M_PI * freq * i / SAMPLE_RATE)); // prev. i used sample count which led to every tone sounding the same
		}
		else{
			// here it pauses. buffer already 0 due to calloc
			sample = 0;
		}
		...
		/*
			...
			Info: For this comment i used claude, it is used to give me a better understanding of what the following two lines from the ao_example.c are doing.
		*/
		buffer[4*i]   = buffer[4*i+2] = sample & 0xff;
		buffer[4*i+1] = buffer[4*i+3] = (sample >> 8) & 0xff;
	}
 	ao_play(device, buffer, bufferSize);
    free(buffer); // america buffer
}
