...
int main(int argc, char *argv[]) {
    ...
    // FIFO buffers decoded samples - MP3 outputs 1152 samples/frame, AAC needs exactly 1024
...
// init AAC encoder, match sample rate and channel layout from decoder
...
AVCodecContext *createEncoder(AVFormatContext *outFmtCtx, AVCodecContext *decCtx) {
    ...
    ctx->sample_fmt = AV_SAMPLE_FMT_FLTP; // enc->sample_fmts deprecated in FFmpeg 8 - AAC always uses FLTP anyway
...
void flushFifo(AVAudioFifo *fifo, AVCodecContext *encCtx, AVFormatContext *outFmtCtx, AVStream *outStream, AVPacket *outPkt, int64_t *pts, int flush) {
    while (av_audio_fifo_size(fifo) >= encCtx->frame_size || (flush && av_audio_fifo_size(fifo) > 0)) {
        ...
        *pts += readSamples;
        ...
        static int dbgFifo = 0;
        if (dbgFifo++ < 2) {
            float *s = (float *)encFrame->data[0];
            fprintf(stderr, "DEBUG fifo out: fmt=%d samples=%d [0]=%.4f [1]=%.4f [2]=%.4f\n",
                encFrame->format, readSamples, s[0], s[1], s[2]);
        }
...
void encodeAndWrite(AVCodecContext *encCtx, AVFormatContext *outFmtCtx, AVStream *outStream, AVFrame *frame, AVPacket *outPkt) {
    int sendRet = avcodec_send_frame(encCtx, frame); // skip invalid frames - MP3 encoder delay at start causes NaN/Inf samples, which the AAC encoder rejects
    static int dbgSend = 0;
    if (dbgSend++ < 5) {
        fprintf(stderr, "DEBUG send_frame ret=%d (frame=%s)\n", sendRet, frame ? "data" : "NULL");
    }
    if (sendRet < 0) {
        return;
    }
